...
export default function ConfigModal({ open, settings, onClose, onSave, saving }) {
  ...
  useEffect(() => {
    const parsed = parseAtlasChartsEmbedUrl(settings?.atlasChartsTowerMapEmbedUrl ?? "");
    setMapBaseUrl(parsed.baseUrl);
    setMapChartId(parsed.chartId);
    setMapEmbedUrl(parsed.embedUrl);
    setDashboardUrl(settings?.atlasChartsTowerDashboardEmbedUrl ?? "");
    setError("");
  }, [settings, open]);
  ...
  function handleSave() {
    ...
    onSave({
      atlasChartsTowerMapEmbedUrl: resolvedMapUrl,
      atlasChartsTowerDashboardEmbedUrl: dashboardUrl.trim()
    });
  }
